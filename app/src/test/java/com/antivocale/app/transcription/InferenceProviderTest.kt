...
class InferenceProviderTest {
    ...
    fun `options contains auto nnapi cpu`() {
        // NNAPI is offered on every device (issue #26 re-enable): the options list
        // must not depend on the SoC, or the dropdown hides it on MediaTek phones
        // while an already-saved preference still resolves to it.
